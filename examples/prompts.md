...
分析服务／行业页、内容、技术问题、可信度及转化路径，输出完整 HTML、
页面清单和 3 个页面草稿。保留网站现有优点，所有建议说明证据和验收方法。
不修改网站，不联络任何机构。
```
...
Show exact fields, environment, publication state, preview method and rollback.
Do not execute any CMS writes until I approve the specific batch.
```
...
every keyword. Keep public estimates separate from actual product analytics.
Deliver analysis and drafts only.
```
...
不读取个人健康记录，不提交表单，不把竞品效果承诺复制为我们的承诺。
输出详细建议、3 个草稿和自包含 HTML，暂不实施。
```
...
and label actual click/conversion metrics unavailable. Do not alter the site or
schedule recurring monitoring. Explain what improved, what remains uncertain,
and which next actions need approval.
```

## Scope controls

- “Diagnose” means observe and explain, not fix.
- “Draft/preview” means prepare proposals; CMS writes still need the stated approval gate.
- “Implement” applies only to the authorized batch, not all future suggestions.
- “Validate” compares evidence; it does not itself create an automation.
- A reference report is coverage/design inspiration, not an additional evidence source.
